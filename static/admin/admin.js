...
(function () {
  ...
  async function apiRequest(endpoint, method = 'GET', body = null) {
    ...
    // --- GitHub Pages / Static Cloud Mode ---
    ...
    if (endpoint === '/api/content' && method === 'GET') {
      const draft = localStorage.getItem('rk_portfolio_draft');
      if (draft) {
      ...
      throw new Error('Could not load portfolio data file.');
    }

    // Local client-side modification for write actions on GitHub Pages
    if (!currentData) {
      currentData = JSON.parse(localStorage.getItem('rk_portfolio_draft') || '{}');
    }

    if (endpoint.startsWith('/api/projects')) {
      if (method === 'POST') {
        const newProj = { id: `project-${Date.now()}`, ...body };
        currentData.projects = currentData.projects || [];
        currentData.projects.push(newProj);
      } else if (method === 'PUT') {
        const id = endpoint.replace('/api/projects/', '');
        const idx = (currentData.projects || []).findIndex(p => p.id === id);
        if (idx !== -1) currentData.projects[idx] = { ...currentData.projects[idx], ...body };
      } else if (method === 'DELETE') {
        const id = endpoint.replace('/api/projects/', '');
        currentData.projects = (currentData.projects || []).filter(p => p.id !== id);
      }
    } else if (endpoint.startsWith('/api/experience')) {
      if (method === 'POST') {
        const newExp = { id: `exp-${Date.now()}`, ...body };
        currentData.experience = currentData.experience || [];
        currentData.experience.push(newExp);
      } else if (method === 'PUT') {
        const id = endpoint.replace('/api/experience/', '');
        const idx = (currentData.experience || []).findIndex(e => e.id === id);
        if (idx !== -1) currentData.experience[idx] = { ...currentData.experience[idx], ...body };
      } else if (method === 'DELETE') {
        const id = endpoint.replace('/api/experience/', '');
        currentData.experience = (currentData.experience || []).filter(e => e.id !== id);
      }
    } else if (endpoint.startsWith('/api/gists')) {
      if (method === 'POST') {
        const newGist = { id: `gist-${Date.now()}`, ...body };
        currentData.gists = currentData.gists || [];
        currentData.gists.push(newGist);
      } else if (method === 'PUT') {
        const id = endpoint.replace('/api/gists/', '');
        const idx = (currentData.gists || []).findIndex(g => g.id === id);
        if (idx !== -1) currentData.gists[idx] = { ...currentData.gists[idx], ...body };
      } else if (method === 'DELETE') {
        const id = endpoint.replace('/api/gists/', '');
        currentData.gists = (currentData.gists || []).filter(g => g.id !== id);
      }
    } else if (endpoint.startsWith('/api/achievements')) {
      if (method === 'POST') {
        const newAch = { id: `ach-${Date.now()}`, ...body };
        currentData.achievements = currentData.achievements || [];
        currentData.achievements.push(newAch);
      } else if (method === 'PUT') {
        const id = endpoint.replace('/api/achievements/', '');
        const idx = (currentData.achievements || []).findIndex(a => a.id === id);
        if (idx !== -1) currentData.achievements[idx] = { ...currentData.achievements[idx], ...body };
      } else if (method === 'DELETE') {
        const id = endpoint.replace('/api/achievements/', '');
        currentData.achievements = (currentData.achievements || []).filter(a => a.id !== id);
      }
    } else if (endpoint === '/api/profile' && method === 'PUT') {
      currentData.profile = { ...currentData.profile, ...body };
    }

    localStorage.setItem('rk_portfolio_draft', JSON.stringify(currentData));
    return { success: true };
  }
  ...
  function exportJson() {
    if (!currentData) {
      showToast('No portfolio data available to export.', 'error');
      return;
    }
    ...
    showToast('Downloaded portfolio-data.json successfully!', 'success');
  }
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (isLocal) {
        ...
        // Cloud mode on GitHub Pages
        if (username.toLowerCase() === 'admin' || password.length >= 6) {
          setToken('cloud_session_' + Date.now());
        } else {
      ...
      showLoginOverlay(false);
      showToast('Welcome to Admin Studio!', 'success');
      await loadContent();
    } catch (err) {
